...
import { ResponsiveCard, ResponsiveGrid } from "../../../../utilities/templates.jsx";
...
export function PlanCards(props) {
	...
	return <div className={"flex flex-col justify-center items-center gap-5".classNames()}>
		...
		<ResponsiveGrid max={3}>
			{content.variations.map(variation=>{
				let {variation_id, label, period_label, price, key_features=[]} = variation;
				return <ResponsiveCard key={variation_id}>
					<div className={"h-full flex flex-col justify-between items-center bg-brand-tertiary shadow-lg rounded-3xl px-14 py-8".classNames()}>
