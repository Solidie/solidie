...
import SubscriptionDetailTable from "./SubscriptionDetailTable.jsx";


const SubscriptionTable = () => {
  return (
    <div className={"flex flex-col w-full gap-4 min-h-max".classNames()}>
        ...
        {Array(20)
          ...
          .map((item, idx) => (
            <AccordionInner
              ...
              header={<Row idx={idx} />}
              idx={idx}
              content={<SubscriptionDetailTable />}
            />
          ))}
      </AccordionPrimitive.Root>
    </div>
...
const Row = ({ idx }) => {
  return (
    <div className={"flex-grow flex-wrap gap-5 text-base flex items-center justify-between w-full bg-tertiary/10 rounded-lg px-4 py-4 transition-all delay-75 ".classNames()}>
